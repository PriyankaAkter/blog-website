...
const HomeCategory = () => {
  let uniqueObjArray = [...new Map(blogs.map((item) => [item["slug"], item])).values()];
       ...
       <div>
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-4">
        {
            uniqueObjArray.map((item,index)=>(
              <Link href={`/category/${item.slug}`} className={`p-8 border border-theme-medium-gray border-opacity-30 ${index===1 && "bg-secondary border-none"}`}>
